...
function ComingSoonCard({ className = "" }: { className?: string }) {
  return (
    <div
      className={`relative rounded-2xl bg-cream border-2 border-dashed border-navy/20 flex items-center justify-center ${className}`}
    >
      <p className="text-sm font-medium text-navy/40 uppercase tracking-widest">
...
export default function Portfolio() {
  ...
  // Build grid slots: max 5 visible cards, pad with null for "coming soon"
  const slots: Array<(typeof PORTFOLIO_ITEMS)[0] | null> = [
    ...filtered.slice(0, 5),
    ...Array(Math.max(0, 5 - filtered.length)).fill(null),
  ];
  ...
  return (
    <section id="portfolio" className="bg-cream py-20 md:py-32 px-6 lg:px-12">
      <div className="max-w-7xl mx-auto flex flex-col gap-12">
        ...
        {/* Bento grid */}
        <div
          className="fade-up grid grid-cols-3 grid-rows-2 gap-4 h-[600px] md:h-[700px]"
          style={{ transitionDelay: "150ms" }}
        >
          {slots.map((item, i) => {
            const isFeatureSlot = i === 0;
            const className = isFeatureSlot ? "col-span-2 row-span-2" : "";
            if (!item) return <ComingSoonCard key={i} className={className} />;
            return (
              <PortfolioCard key={item.title} item={item} className={className} />
            );
          })}
        </div>
      </div>
